...
def log_coffee():
    ...
    if 'user' in st.session_state:
        with conn.session as session:
            log = Log(user=st.session_state.user, ts=datetime.now().isoformat(), anzahl=st.session_state.anzahl_slider)
            session.add(log)
            session.commit()
...
def check_user():
    with conn.session as session:
        try:
            user = session.query(User).filter(User.code == sha256(st.session_state.code_input.encode('utf-8')).hexdigest(), User.status == 'active').first()
...
def get_subscription_fee():
    ...
    with conn.session as session:
        num_members = session.execute(
            select(User).where(User.mitglied == True)
        )
...
with st.form(key='log_coffee', clear_on_submit=True):
    anzahl = st.select_slider('Wieviele Tassen Kaffee wollen Sie eintragen?', options=list(range(1,6)), key='anzahl_slider')
    code = st.text_input("Geben Sie Ihr Kennwort ein.", key='code_input')
    submit = st.form_submit_button("Kaffee eintragen", type='primary', on_click=check_user)
...
with st.expander('Kennwort vergessen?'):
    ...
    if st.button("Kennwort zurücksetzen"):
        ...
        st.write(f"Ein Link zum Zurücksetzen Ihres Kennworts wurde an {email} gesendet.")
